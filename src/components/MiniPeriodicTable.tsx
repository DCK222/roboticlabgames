...
interface Element {
  z: number; sym: string; name: string; mass: string; category: string; group: number; period: number; color: string;
}
...
const CATEGORY_COLORS: Record<string, string> = {
  "Alcalino": "bg-red-500/40", "Alcalinotérreo": "bg-orange-500/40", "Transición": "bg-sky-500/40",
  "Metal": "bg-blue-500/40", "Metaloide": "bg-teal-500/40", "No metal": "bg-green-500/40",
  "Halógeno": "bg-yellow-500/40", "Gas noble": "bg-purple-500/40",
};

const MiniPeriodicTable = () => {
    ...
    <div>
      <div className="flex flex-wrap gap-1 justify-center mb-2">
        {Object.entries(CATEGORY_COLORS).map(([cat, clr]) => (
          <span key={cat} className={`${clr} rounded px-1.5 py-0.5 text-[8px] font-semibold text-foreground`}>{cat}</span>
        ))}
      </div>
      <div className="w-full overflow-x-auto flex justify-center pb-2">
        <div className="inline-grid gap-[1px]" style={{ gridTemplateColumns: "repeat(18, 1fr)" }}>
          {grid.map((row, ri) =>
            row.map((el, ci) => {
              if (!el) return <div key={`${ri}-${ci}`} className="w-5 h-5 sm:w-7 sm:h-7" />;
              const isSelected = selected?.z === el.z;
              return (
                <button
                  key={el.z}
                  onClick={() => setSelected(isSelected ? null : el)}
                  className={`w-5 h-5 sm:w-7 sm:h-7 rounded-sm flex flex-col items-center justify-center transition-all border ${
                    isSelected ? "neon-border scale-110 z-10" : "border-border/50 hover:border-primary/50"
                  } ${CATEGORY_COLORS[el.category] || "bg-secondary"}`}
                >
                  <span className="text-[5px] sm:text-[6px] text-muted-foreground leading-none">{el.z}</span>
                  <span className="text-[7px] sm:text-[9px] font-bold text-foreground leading-none">{el.sym}</span>
                </button>
              );
            })
          )}
      ...
      {selected && (
        <div className="glass-card rounded-lg p-3 mt-2 text-center">
          <span className="text-2xl font-bold text-primary">{selected.sym}</span>
          <span className="ml-2 font-semibold text-foreground">{selected.name}</span>
          <span className="ml-2 text-xs text-muted-foreground">Z={selected.z} · {selected.mass} u</span>
          <span className={`ml-2 ${CATEGORY_COLORS[selected.category]} rounded px-2 py-0.5 text-[10px] font-semibold`}>{selected.category}</span>
          <button onClick={() => setSelected(null)} className="ml-2 text-xs text-muted-foreground hover:text-foreground">✕</button>
        </div>
      )}
    </div>
  );
